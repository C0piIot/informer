    ...
    def post(self, request, *args, **kwargs):
        with transaction.atomic():
            ...
            if target_step := source_step.prev_step() if 'up' in request.POST else source_step.next_step():
                ...
                self.flow.steps.filter(order=target_step.order).update(order=source_step.order)
        ...
        return HttpResponseRedirect(reverse('flows:edit', kwargs={'id': self.flow.id, 'environment': self.current_environment.slug }))
