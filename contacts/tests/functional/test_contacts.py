    ...
    def testEnvironmentCrud(self):
        ...
        with self.settings(ALLOWED_HOSTS=("example.com",), CONTACT_STORAGE=ContactsConfig.DEFAULT_SETTINGS['CONTACT_STORAGE']):
            ...
            response = self.client.post(
                reverse("contacts:contact_create", kwargs={
            ...
            response = self.client.post(
                reverse("contacts:contact_update", kwargs={
                        'environment': self.environment.slug, 'key': 123}),
                {
            ...
            self.assertRedirects(
                self.client.get(
                    reverse("contacts:contact_remove", kwargs={
                            'environment': self.environment.slug, 'key': 123}),
                    HTTP_HOST="example.com"
                ),
                reverse("contacts:list", kwargs={
